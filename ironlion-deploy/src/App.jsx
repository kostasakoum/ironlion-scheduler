...
const CALENDAR_ASSESSMENTS = {
  ...
  "2026-04-16": [
    { hour: 18, members: [null, "Kiana"] }
  ],
  "2026-04-23": [
    { hour: 9, member: "Alejandra" }
  ]
};
...
function buildHourAssignment(dayName, hour, members, total, customLayout, mondayFallback, ignoreZoneCap, absentSet) {
  ...
  const semi = members.filter(m => !m.isFoundations && !m.isOpenGym && !m.isInferno && !m.isBodiesInMotion);
...
export default function GymScheduler() {
  ...
  const buildAndApplyShift = useCallback((detectedDay, parsed, shift, dateObj) => {
    ...
    if (dateObj) {
      ...
      if (CALENDAR_ASSESSMENTS[dateStr]) {
        const asmts = CALENDAR_ASSESSMENTS[dateStr];
  ...
  const processFile = useCallback(async (file) => {
    ...
    try {
      ...
      for (let i = headerIdx+1; i < rows.length; i++) {
        ...
        if (firstName) parsed.push({ hour, firstName, lastName, isFoundations, isOpenGym, isInferno, isBodiesInMotion, isCarlsen });
      }
      ...
      if (dateStr && CALENDAR_ASSESSMENTS[dateStr]) {
        const asmts = CALENDAR_ASSESSMENTS[dateStr];
        const newAssessments = {};
        const newAssessmentNames = {};
        asmts.forEach(a => {
                ...
                <thead>
                  <tr>
                    ...
                    {cfg.hours.map(hour => {
                      ...
                      const classMembers = entries ? entries.filter(e => e.hour === hour && (e.isInferno || e.isBodiesInMotion)).length : 0;
